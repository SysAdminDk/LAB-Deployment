...
Get-ChildItem -Path "$(Split-Path -Path $RootPath)\Functions" | ForEach-Object { Import-Module -Name $_.FullName -Force }
...
if ($null -eq $VHDDrive) {
    ...
    New-Unattend -ComputerName $VMName -RunAtStartup "BootStrap.ps1" -ProductKey $ProductKey -AdminUsername $LocalUsername -AdminPassword $LocalPassword -DiskId $null | `
    ...
    Copy-Item -Path "D:\Deployment\Server Reference\Windows unattend\PVE-Bootstrap.ps1" -Destination "$VHDXVolume3\TS-Data\BootStrap.ps1"
