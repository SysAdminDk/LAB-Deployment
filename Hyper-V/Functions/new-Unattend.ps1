function New-Unattend {
    ...
    if ($Interfaces) {
        $UnattendXml += "        <component name=`"Microsoft-Windows-NetTCPIP`" processorArchitecture=`"amd64`" publicKeyToken=`"31bf3856ad364e35`" language=`"neutral`" versionScope=`"nonSxS`">"
        $UnattendXml += "            <Interfaces>"


        $UnattendXml += "                <Interface wcm:action=`"add`">"
        $UnattendXml += "                    <Identifier>$($Interfaces | Select-Object -First 1)</Identifier>"
        ...
        if ( ($IPAddress) -and ($DNSServers) -and ($Gateway) ) {
            ...
            $UnattendXml += "                    </UnicastIpAddresses>"
            $UnattendXml += "                    <DefaultGateways>"
            $UnattendXml += "                        <Gateway wcm:action=`"add`" wcm:keyValue=`"1`">$Gateway</Gateway>"
            $UnattendXml += "                    </DefaultGateways>"
            ...
            $UnattendXml += "                    </DNSServerSearchOrder>"
            $UnattendXml += "                    <EnableAdapterDomainNameRegistration>false</EnableAdapterDomainNameRegistration>"
        ...
        foreach ($Interface in $Interfaces | Select-Object -Skip 1) {

            $UnattendXml += "                <Interface wcm:action=`"add`">"
            $UnattendXml += "                    <Identifier>$Interface</Identifier>"
            $UnattendXml += "                    <DhcpEnabled>false</DhcpEnabled>"
            $UnattendXml += "                    <EnableAdapter>false</EnableAdapter>"
            $UnattendXml += "                </Interface>"
        }
