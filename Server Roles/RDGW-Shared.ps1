...
# ------------------------------------------------------------
$RDGWServers = $($ImportServerInfo | Where {$_.Name -match "^RDGW-1[0-9]{1}$"}).Name | Get-ADComputer -ErrorAction SilentlyContinue
...
#Get-NlbCluster -HostName $RDGWServers[0].DNSHostName | FL
#Get-NlbClusterNode -HostName $RDGWServers[0].DNSHostName
