...
<#
    ...
    Install & Configure Tier E Management Servers

#>

# Select management servers for tier Endpoints
# ------------------------------------------------------------
$TierEJumpStations = $($ServerInfo | Where {$_.Role -eq "MGMT"}) | Out-GridView -Title "Select the Tier Endpoint Jump / Management Servers" -OutputMode Multiple

# Install RSAT tools on Tier Endpoint
# ------------------------------------------------------------
$($TierEJumpStations).Name | Get-ADComputer -ErrorAction SilentlyContinue | Foreach {

    If ( ($Null -ne $TargetPath) -and ($($_.DistinguishedName) -NotLike "*OU=JumpStations,OU=TierEndpoints*") ) {
